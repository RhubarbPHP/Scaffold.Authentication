...
class UserPastPasswordTest extends RhubarbTestCase
{
    ...
    public function testRemovePastPasswords()
    {
        ...
        for ($i = 0; $i < 30; $i++) {
            $pastPassword = new UserPreviousPassword();
            $pastPassword->UserID = 23;
            $pastPassword->Password = 'testingthisshouldbeahash';
            $pastPassword->save();
        }

        $this->assertEquals(UserPreviousPassword::find(new Equals("UserID", 23))->count(), 30);
        UserPreviousPassword::removePreviousPasswords(23);
        $this->assertEquals(UserPreviousPassword::find(new Equals("UserID", 23))->count(), AuthenticationSettings::singleton()->totalPreviousPasswordsToStore - 1);
    }
    ...
    public function testMaxPastPasswordsToStore()
    {
        ...
        $this->assertEquals(UserPreviousPassword::find(new Equals("UserID", $user->UniqueIdentifier))->count(), 30);
        $user->setNewPassword("newjazzypassword");
        $user->save();

        $this->assertEquals(UserPreviousPassword::find(new Equals("UserID", $user->UniqueIdentifier))->count(), AuthenticationSettings::singleton()->totalPreviousPasswordsToStore);
    }
    ...
    public function testDisableStorePastPasswords()
    {
        ...
        $this->assertEquals(UserPreviousPassword::find(new Equals("UserID", $user->UniqueIdentifier))->count(), 0);
